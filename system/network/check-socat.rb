...
metadata:
  ...
  namespace: default
spec:
  command: check-socat.rb -t {{ .labels.check_socat_timeout | default 5s }} -i {{ .labels.check_socat_input | default "UDP4-RECVFROM:5555,ip-add-membership=224.0.0.1:eth0" -o {{ .labels.check_socat_output | default /dev/null }}
  interval: 10
  publish: true
  runtime_assets:
